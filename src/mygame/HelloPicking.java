...
public class HelloPicking extends SimpleApplication {
  ...
  public void simpleInitApp() {
    ...
    cam.setLocation(cam.getLocation().add(0f, 0f, 15f));
    cam.update();
  }
  ...
  private float movimientoDireccion = 1f;
   ...
   public void simpleUpdate(float tpf) {
        //yellow.move(2, 2, 2); 
        movimiento += tpf * movimientoDireccion*10f;
        
        if (movimiento < 2f || movimiento > 20f){   
            movimientoDireccion = movimientoDireccion * -1f;
            if (movimiento < 2f){
                movimiento = 2f;
            }else if (movimiento > 20f){
                movimiento = 20f;
            }
        }
        
        boxes[0].setLocalTranslation(-2+movimiento, 0f, 1f); 
